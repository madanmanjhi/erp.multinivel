...
class registro {
    ...
    function isVIP($id){
            $query = "SELECT 
                        ...
                        FROM
                            ...
                            venta v
                        WHERE
                            i.id = m.id AND m.id = cvm.id_mercancia
                                AND cvm.id_venta = v.id_venta
                                AND m.id_tipo_mercancia IN (5)
                                AND v.id_user IN ($id)
        ...
        function getDirectos($id){
            $q =newQuery($this->db,"SELECT distinct id_afiliado as id"
                    . " FROM afiliar WHERE directo = $id");
            
            return $q;
        }
        
	function isBonusVIP($id){               
            $VIP = 0;
            
            $directos = $this->getDirectos($id);
            if($directos){                
                foreach ($directos as $dato){
                    $id_vip = $dato["id"];
                    $isVIP = $this->isVIP($id_vip);
                    if($isVIP)
                        $VIP++;
                }
            }
        ...
        {
            $q = newQuery($this->db,"SELECT * FROM mercancia WHERE id = $id");        
            
            return $q  ? $q[1] : false;
        }         
        
        private function insertVentaItem($id_venta,$item,$precio = "",$cantidad = 1)
        {                    
            if($precio===""){
                $merc = $this->getMercancia($item);
                $precio = $merc ? $merc["costo"] : 0;
            }                               
            
            $total = $cantidad*$precio;

            $query = "INSERT INTO cross_venta_mercancia 
                        VALUES ($id_venta,$item,$cantidad,$precio,0,$total,'',null)";
        ...
        {                      
                ...
                return $this->insertValues($dato, "venta","id_venta");
                
        }
                        
        function newVenta($id,$item,$precio="",$cantidad = 1){
            
            $id_venta = $this->insertVenta($id);
            if(gettype($item)=="array"){
                $i=0;
                foreach ($item as $value) {
                     $this->insertVentaItem($id_venta, $value, $precio[$i],$cantidad[$i]);
                     $i++;
                }
            }else{
                $this->insertVentaItem($id_venta, $item, $precio,$cantidad);
            }
            
        }
        
        function checkVIP($id,$vip){
            $isVIP = $this->isBonusVIP($id);
            if($isVIP)
                $this->newVenta($vip, 4, 0);
        }
    
    function insertValues($data,$table,$last = "id") {
        $values = array();
        $fields = array();
        foreach ($data as $key => $value) {
            $valor = (gettype($value) == "string") ? "'" . $value . "'" : $value;
            array_push($values, $valor);
        ...
        $insert_id= newQuery($this->db,"select $last as id from $table order by $last DESC LIMIT 1");
        return $insert_id ? $insert_id[1]["id"] : 1;
    ...
    function crearUsuario(){
       ...
       /* echo "red : ".$mi_red
        ...
        #foreach (array($dato_perfil,$dato_afiliar,$dato_estilo,$dato_dir,$dato_coaplicante,$dato_billetera,$dato_rango,$dato_img) as $rest){
            #echo "<br/><br/>";var_dump($rest);
        #}               
